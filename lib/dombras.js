...
// "от 65 000 ₸" по-русски/английски — слово стоит ПЕРЕД числом.
// "65 000 ₸ бастап" по-казахски, "65.000 ₸ başlangıç" по-турецки —
// в этих языках такое слово ("начиная с") стоит ПОСЛЕ числа, это
// грамматическое правило, а не опечатка. Раньше по всему сайту было
// жёстко зашито "priceFrom + цена" — верно только для ru/en, на kz/tr
// получалось "бастап 65 000 ₸", что звучит неправильно. Эта функция
// собирает строку в правильном порядке под каждый язык.
export function formatPriceFrom(priceFrom, priceText, locale) {
  if (locale === "kz" || locale === "tr") {
    return `${priceText} ${priceFrom}`;
  }
