...
// • silverOption/decorationOption убраны совсем — Айым убирает
//   соответствующие поля has_silver_option/has_decoration_option
//   из Supabase, они больше не нужны.
...
const CHARACTERISTICS_BY_LOCALE = {
  ...
  kz: [
    ...
    { label: "Шанақ", value: "жаңғақ немесе үйеңкі" },
    { label: "Мойын", value: "үйеңкі" },
    { label: "Мойын үстіндегі жапсырма", value: "абанос" },
    { label: "Басы", value: "корпус түсіндегі массив" },
    { label: "Құлақ", value: "жабық, механикалық" },
    { label: "Ішектер", value: "карбон" },
    { label: "Шпон ою", value: "табиғи, боялмаған" },
  ],
  en: [
...
// Остальной текст страницы — тоже прямо здесь, на 4 языках, без dict:
// подпись "Размер", заголовок и примечание блока характеристик, и сами
// 5 подписей в плашках "бесплатно".
const PAGE_TEXT_BY_LOCALE = {
  ru: {
    sizeLabel: "Размер",
    specsLabel: "Характеристики — базовая комплектация",
    specsNote:
    ...
    freebies: ["Мягкий чехол", "Доставка по всему миру", "Гарантия 3 года", "Сертификат настоящего казаха", "Стикерпак"],
    orderCta: "Заказать / уточнить цену",
    orderPrefill: "Здравствуйте! Интересует домбра «{name}».",
  },
  kz: {
    sizeLabel: "Өлшем",
    specsLabel: "Сипаттамалар — базалық жинақ",
    specsNote:
      "Бұл домбраның базалық түрі. Басқа ағаштан жасауға, корпустың бүйіріне өрнек қосуға немесе күміс қаптама тапсыруға болады — шебермен талқылаңыз.",
    freebies: ["Жұмсақ қап", "Әлем бойынша жеткізу", "3 жылдық кепілдік", "Нағыз қазақ сертификаты", "Стикерпак"],
    orderCta: "Тапсырыс беру / бағасын сұрау",
    orderPrefill: "Сәлеметсіз бе! «{name}» домбырасы қызықтырады.",
  },
  en: {
    sizeLabel: "Size",
    specsLabel: "Specifications — base configuration",
    specsNote:
      "This is the base version of the dombra. It can be made from a different wood, with side patterns or a silver plate added — discuss with the master.",
    freebies: ["Soft case", "Worldwide delivery", "3-year warranty", "Certificate of a true Kazakh", "Sticker pack"],
    orderCta: "Order / ask the price",
    orderPrefill: "Hello! I'm interested in the «{name}» dombra.",
  },
  tr: {
    sizeLabel: "Ölçü",
    specsLabel: "Özellikler — temel yapılandırma",
    specsNote:
      "Bu dombranın temel halidir. Farklı bir ağaçtan yapılabilir, gövdenin yanlarına desen veya gümüş kaplama eklenebilir — ustayla görüşün.",
    freebies: ["Yumuşak kılıf", "Dünya çapında teslimat", "3 yıl garanti", "Gerçek Kazak sertifikası", "Stikerpak"],
    orderCta: "Sipariş ver / fiyat sor",
    orderPrefill: "Merhaba! «{name}» dombrasıyla ilgileniyorum.",
  },
};
...
export default async function DombraPage({ params }) {
  ...
  const t = PAGE_TEXT_BY_LOCALE[locale] || PAGE_TEXT_BY_LOCALE.ru;
  const name = localizedDombraName(item, locale);
  const description = localizedDombraDescription(item, locale);
  ...
  const whatsappMessage = encodeURIComponent(t.orderPrefill.replace("{name}", name));
    ...
    <main className="w-full px-5 py-10 lg:mx-auto lg:max-w-[1180px] lg:px-7 lg:py-16">
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-14">
        ...
        <div>
          ...
          {/* характеристики — базовая комплектация */}
